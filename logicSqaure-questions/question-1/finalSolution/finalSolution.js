...
function searchcafe(cafeName) {
    let cafedetails = [...cafes];
    let searchresult = [];
    let placesdetails = [...places];
    cafedetails.forEach((obj) => {
        if (obj.name.toLowerCase().includes(cafeName.toLowerCase())) {
            placesdetails.forEach((cafedetail) => {
                if (cafedetail.id === obj.place_id) {
                    let tempobj = {
                        "id": cafedetail.id,
                        "street_no": cafedetail.street_no,
                        "locality": cafedetail.locality,
                        "postal_code": cafedetail.postal_code,
                        "lat": cafedetail.lat,
                        "long": cafedetail.long
                    }
                    let resobj = {
                        "name": obj.name,
